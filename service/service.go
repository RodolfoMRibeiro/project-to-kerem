...
func (m mongoService) InsertInDatabase(c *gin.Context, dSU dto.DtoSignUp) (*mongo.InsertOneResult, error) {
	...
	res, err := m.mongoRepository.InsertNumberInDatabase(c, ctx, &aMap)
	if err != nil {
...
func (mongoService) setValues(person *model.Person) error {
...
func (m mongoService) update(ctx context.Context, foundPerson model.Person) error {
...
func (m mongoService) FindInDatabase(c *gin.Context, dLI dto.DtoLogIn) (*model.Person, error) {
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	defer cancel()
...
func (m mongoService) GetFromDatabase(c *gin.Context, dGU dto.GetUser, personId string) (model.Person, error) {
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	defer cancel()
...
func (m mongoService) GetallFromDatabase(c *gin.Context, allUsers []primitive.M) ([]primitive.M, error) {
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	defer cancel()
	...
	m.mongoRepository.Results(c, ctx).All(ctx, &allUsers)
	return allUsers, nil
}
